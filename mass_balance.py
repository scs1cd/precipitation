...
def run_massbalance(M, K, Tcmb, var, verb):
    ...
    # get_mole returns: [XFe, XO, XSi, XMg, XFeO, XMgO, XSiO2]
    # mole2mass_core takes: XFe, XO, XSi, XMg
    # mole2mass_mant takes: XFeO, XSiO2, XMgO
    iX = get_molefractions(M) 
    cO_c_init  , cSi_c_init  , cMg_c_init  = mole2mass_core(iX[0], iX[1], iX[2], iX[3]) 
    cFeO_m_init, cSiO2_m_init, cMgO_m_init = mole2mass_mant(iX[4], iX[6], iX[5]) 
    ...
    for t in Tcmb:         
        ...
        # Set new initial condition - only relevant in Kd are composition-dependent
...
def print_init_conc(X,c):
    ...
    print('Initial core mole fractions of Fe  = {:6.2f} O   = {:6.2f} Si   = {:6.2f} Mg  = {:6.2f} All = {:6.2f}'
...
def print_any_conc(X,c,loc):
    ...
    print('Core mole fractions of Fe  = {:6.2f} O   = {:6.2f} Si   = {:6.2f} Mg  = {:6.2f} All = {:6.2f}'
...
def plot_all(Tcmb, M, X, c, loc):
    ...
    ax2.plot(Tcmb[Mgloc], XMg[Mgloc]   , color='red'   , marker='o')
    ax2.plot(Tcmb[Oloc] , XO[Oloc]     , color='blue'  , marker='o')
    ax2.plot(Tcmb[Siloc], XSi[Siloc]   , color='orange', marker='o')
    ax2.legend(loc=2)

    dcdT_MgO = np.gradient(cMgO , dT)*1e5
    dcdT_FeO = np.gradient(cFeO , dT)*1e5
    dcdT_SiO2= np.gradient(cSiO2, dT)*1e5
    dcdT_Mg  = np.gradient(cMg, dT)*1e5
    ...
    ax4.plot(Tcmb, dcdT_O  , color='blue'  , label="O"  , linestyle=':')
    ax4.plot(Tcmb[Mgloc], dcdT_Mg[Mgloc]   , color='red'   , marker='o')
    ax4.legend(loc=2)
    
# M = [iMFe, iMO, iMSi, iMMg, iMFeO, iMMgO, iMSiO2]
# get_molefractions(M) Returns [XFe, XO, XSi, XMg, XFeO, XMgO, XSiO2]
def plot_paper(Tcmb, M, X, c, loc):
    ...
    ax4.plot(Tcmb, dcdT_Mg  , color='red'   , label="Mg"      , linestyle='-')
    ax4.plot(Tcmb, dcdT_O   , color='blue'  , label="O"       , linestyle='-')    
    ax4.plot(Tcmb, dcdT_Si  , color='orange', label="Si"      , linestyle='-')    
    ax4.plot(Tcmb[Mgloc], dcdT_Mg[Mgloc]   , color='red'   , marker='o')
